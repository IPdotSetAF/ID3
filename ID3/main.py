...
print('\n\nstarting lazy learning\n')
#testing resolve
for i in range(len(_testData)):
    print(f'{_testData[i]} , {_testTargets[i]}')
    result = id3.Resolve(_features, _testData[i], _testTargets[i])
    print(result)
    if(result == 'ID3.badClassified'):
        ID3_Drawer(id3).Draw()

print('\n\nfinished lazy learning\nevaluating all data\n')
    
for i in range(len(_dataSet.data)):
    print(f'{_dataSet.data[i]} , {_dataSet.targets[i]}')
    print(id3.Resolve(_features, _dataSet.data[i], _dataSet.targets[i]))    